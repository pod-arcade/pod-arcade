...
- [MQTT](#mqtt)
  ...
  - [Desktop APIs](#desktop-apis)
    ...
    - [`desktops/{desktop-id}/ice-servers`](#desktopsdesktop-idice-servers)
...
#### `desktops/{desktop-id}/ice-servers`

This message contains a json array of ice servers that can be used by a client in conjunction with the `server/ice-servers` to establish a WebRTC connection. These ice servers are any additional ice servers that the server was provided with during configuration. The payload is a JSON array of objects with the following properties:

- `urls`: A string array of of URLs that can be used to connect to the ICE servers
- `username`: An optional username to use when authenticating with the ICE server
- `credential`: An optional set of credentials to use when authenticating with the ICE server. It's usually a string, but in rare cases can be an object when the credentialType is set to `oauth`. If it's an object, it will have the following properties:
  - `macKey`: the mac key used when credentialType is set to `oauth`
  - `accessToken`: the access token used when credentialType is set to `oauth`
- `credentialType`: An optional type of credential that is being used. This is either `password` or `oauth`.

```javascript
[
  // example ICE Servers with no authentication (very common)
  {
    "urls": ["stun:stun1.l.google.com:19302", "stun:stun2.l.google.com:19302"]
  },
  // example ICE Servers with password authentication (commonly)
  {
  ...
  // example ICE Servers with oauth authentication (almost never used)
  {
    "urls": ["turn:turn.example.com:3478"],
    "username": "username",
    "credential": {
